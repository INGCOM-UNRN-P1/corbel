---
title: "Guía de Extensión y Creación de Plugins: corbel"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de corbel"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-corbel-plugins)=
# Guía de Extensión y Plugins: corbel

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`corbel`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-corbel-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`corbel` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para corbel`.
- **Punto de Entrada Oficial**: `corbel.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-corbel-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para corbel."""
    name = "custom_rule"
...
Para que `corbel` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."corbel.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `corbel` la reconozca:
...
(manual-corbel-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `corbel` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = corbel.ejecutar_analisis(
...
(manual-corbel-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `corbel` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar corbel
        run: pip install -e ./corbel
        
      - name: Ejecutar Auditoría
        run: corbel check src/ include/ --json > reporte.json
````

---

(manual-corbel-plugins-ejercicios)=
